...
const devs = ['536565404542173193'];
const db = require('quick.db');
const premium = ['494469169862148116']
...
message.channel.send(
  ...
  .setColor("#9B59B6")
  .addField(" ** :gear: Server Support :gear: **" , "  **https://discord.gg/cXnyGhp**")
...
client.on('message', message => {
    if(message.content == '^vip-servers') {
             if(!message.author.id === '455288398249328650') return;
    var gimg;
    var gname;
    var gmemb;
    var gbots;
...
client.on('guildCreate', guild => {
  ...
  .setDescription(`**
  New Server Add dynamic Community © ?
...
client.on('guildDelete', guild => {
     ...
     .setURL('https://discordapp.com/oauth2/authorize?client_id=435392018693488641&scope=bot&permissions=2146958591')
  .setDescription(`**
  Server Kicked dynamic Community © :cry:
اسم السيرفر: ${guild.name}
صاحب السيرفر: ${guild.owner}**`);
client.channels.get("474127377245667328").sendEmbed(embed)
});
...
client.on('message', message => {
    if(!message.guild) return;
    if (message.author.bot) return;
      let id = message.author.id,prefix="D";//البريفكس
...
const adminprefix = "D";//by ,$ ReBeL ء , ??#4777 'CODES SERVER'
...
if (message.content.startsWith('G.users')){
 if(!message.author.id === '455288398249328654') return;
...
client.on('message', message => {
  ...
  if (message.author.id == 536565404542173193) return;


if (message.content.startsWith(prefix + 'playing')) {
if (message.author.id !== '327080441800556556') return message.reply('** هذا الأمر فقط لصاحب البوت و شكراًً **')
client.user.setGame(argresult);
